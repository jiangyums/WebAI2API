...
export function validateInstancesConfig(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (
            inst.userDataMark !== undefined &&
            inst.userDataMark !== null &&
            inst.userDataMark !== ""
        ) {
        ...
        if (!inst.workers || !Array.isArray(inst.workers)) {
            ...
            for (let j = 0; j < inst.workers.length; j++) {
                ...
                if (!w.name || typeof w.name !== "string") {
                    ...
                    errors.push(
                        `${wPrefix}: Worker 名称 "${w.name}" 全局重复（Worker 名称必须全局唯一）`
                    );
                } else {
                ...
                if (w.type === "merge") {
                    if (
                        !w.mergeTypes ||
                        !Array.isArray(w.mergeTypes) ||
                        w.mergeTypes.length === 0
                    ) {
                        ...
                        for (const mt of w.mergeTypes) {
                            if (!validAdapterTypes.has(mt) || mt === "merge") {
                                errors.push(
                                    `${wPrefix}: mergeTypes 中的 "${mt}" 不是有效的适配器类型`
                                );
                            }
                        }
                    }
                    if (w.mergeMonitor && !w.mergeTypes?.includes(w.mergeMonitor)) {
                        errors.push(
                            `${wPrefix}: mergeMonitor "${w.mergeMonitor}" 必须是 mergeTypes 中的一个`
                        );
                    }
                }
            }
...
export function validatePoolConfig(data) {
    ...
    if (data.failover) {
        ...
        if (data.failover.maxRetries !== undefined) {
            if (
                typeof data.failover.maxRetries !== "number" ||
                !Number.isInteger(data.failover.maxRetries)
            ) {
        ...
        if (
            data.failover.imgDlRetry !== undefined &&
            typeof data.failover.imgDlRetry !== "boolean"
        ) {
            errors.push("failover.imgDlRetry 必须是布尔值");
        }
        if (data.failover.imgDlRetryMaxRetries !== undefined) {
            if (
                typeof data.failover.imgDlRetryMaxRetries !== "number" ||
                !Number.isInteger(data.failover.imgDlRetryMaxRetries)
            ) {
                errors.push("failover.imgDlRetryMaxRetries 必须是整数");
            } else if (
                data.failover.imgDlRetryMaxRetries < 1 ||
                data.failover.imgDlRetryMaxRetries > 10
            ) {
    ...
    if (data.gemini_biz) {
        if (data.gemini_biz.entryUrl !== undefined) {
            if (typeof data.gemini_biz.entryUrl !== "string") {
                ...
                data.gemini_biz.entryUrl &&
                !data.gemini_biz.entryUrl.startsWith("https://")
            ) {
