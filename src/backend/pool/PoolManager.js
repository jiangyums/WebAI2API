...
export class PoolManager {
    ...
    async initAll() {
        ...
        for (const workerConfig of workerConfigs) {
            ...
            if (workerConfig.type !== "merge" && !registry.hasAdapter(workerConfig.type)) {
                logger.error(
                    "工作池",
                    `Worker [${workerConfig.name}] 的类型 "${workerConfig.type}" 无对应适配器，跳过`
                );
                continue;
            }

            if (workerConfig.type === "merge") {
                const invalidTypes = (workerConfig.mergeTypes || []).filter(
                    (t) => !registry.hasAdapter(t)
                );
                if (invalidTypes.length > 0) {
                    logger.error(
                        "工作池",
                        `Worker [${workerConfig.name}] 的 mergeTypes 包含无效类型: ${invalidTypes.join(", ")}`
                    );
                    continue;
                }
            }
        ...
        if (isLoginMode && validWorkers.length === 0) {
            ...
            throw new Error(
                `登录模式未找到 Worker "${loginWorkerName}"。可用的 Worker: ${availableNames}`
            );
        }
            ...
            try {
                ...
                if (existing) {
                    ...
                    if (workerProxy !== existingProxy) {
                        logger.warn(
                            "工作池",
                            `[${worker.name}] 代理配置与 [${existing.firstWorkerName}] 不一致，将使用后者的配置`
                        );
                    }

                    logger.debug(
                        "工作池",
                        `[${worker.name}] 将与其他 Worker 共享浏览器 (${worker.userDataDir})`
                    );
                ...
                logger.error("工作池", `[${worker.name}] 初始化失败，跳过该 Worker`, {
                    error: e.message
                });
            }
        }
        ...
        logger.info(
            "工作池",
            `工作池初始化完成，共 ${this.workers.length} 个 Worker 就绪 (${browserMap.size} 个浏览器实例)`
        );
    }
        ...
        switch (this.strategy) {
            ...
            default: {
                return candidates.reduce(
                    (min, w) => (w.busyCount < min.busyCount ? w : min),
                    candidates[0]
                );
            }
        }
    ...
    async generate(ctx, prompt, paths, modelId, meta) {
        ...
        if (hasImages && candidates.length > 1) {
            ...
            if (optionalCandidates.length > 0) {
                logger.debug(
                    "工作池",
                    `请求包含图片，优先选择支持图片的 Worker (${optionalCandidates.length}/${candidates.length} 个)`
                );
    ...
    async _safeExecuteWorker(worker, ctx, prompt, paths, modelId, meta) {
        try {
            ...
            logger.error("工作池", `[${worker.name}] 执行异常`, {
                error: err.message,
                ...meta
            });
