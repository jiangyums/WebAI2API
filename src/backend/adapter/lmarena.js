...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
    ...
    try {
        ...
        if (httpError) {
            ...
            return {
                error: `请求生成时返回错误: ${httpError.error}`,
        ...
        if (img) {
            ...
            logger.warn("适配器", "未获得结果，响应中无图片数据", {
                ...meta,
                preview: content.substring(0, 150)
            });
            return {
                error: `未获得结果，响应中无图片数据: ${content.substring(0, 200)}`
            };
        }
    } catch (err) {
        // 顶层错误处理
...
export const manifest = {
    ...
    description:
        "使用 LMArena 平台生成图片，支持多种图片生成模型。需要已登录的 LMArena 账户，若不登录会频繁弹出人机验证码且有速率限制。",
