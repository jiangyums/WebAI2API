        ...
        if (currentUrl.includes("auth.business.gemini.google/account-chooser")) {
            ...
            if (submitBtn) {
                ...
                try {
                    ...
                    logger.warn(
                        "适配器",
                        `[登录器(gemini_biz)] 等待跳转回目标页面超时，尝试继续... 当前URL: ${finalUrl}`
                    );
                }
...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
    ...
    try {
        ...
        const targetUrl =
            config.backend?.adapter?.gemini_biz?.entryUrl || config.backend?.geminiBiz?.entryUrl;
            ...
            await pasteImages(
                ...
                imgPaths,
                {
                    uploadValidator: (response) => {
                        ...
                        return (
                            response.status() === 200 && url.includes("global/widgetAddContextFile")
                        );
                    }
                },
                meta
...
export const manifest = {
    ...
    description:
        "使用 Gemini Business 企业版生成文本，支持 Grounding 搜索模式。需要提供入口 URL 并已登录企业账户 (每个谷歌账户首次可以在官网点击免费试用获取30天使用资格)，与 gemini_biz 共享配置。",
