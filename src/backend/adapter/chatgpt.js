    ...
    try {
                    ...
                    uploadValidator: (response) => {
                        ...
                        if (response.status() === 200) {
                            ...
                            if (url.includes("backend-api/files/process_upload_stream")) {
                                ...
                                logger.info(
                                    "适配器",
                                    `图片处理进度: ${processedCount}/${expectedUploads}`,
                                    meta
                                );
        ...
        try {
            ...
            isImageGenerationStarted =
                conversationBody.includes("dalle") || conversationBody.includes("file_");
            ...
            logger.debug(
                "适配器",
                `提取到文本 (${conversationText.length} 字符): ${conversationText.substring(0, 200)}...`,
                meta
            );
        } catch (e) {
        ...
        if (conversationText) {
            ...
            if (isRateLimit) {
                logger.warn(
                    "适配器",
                    `早期检测到速率限制: ${conversationText.substring(0, 200)}...`,
                    meta
                );
                return {
            ...
            if (!isImageGenerationStarted) {
                const isContentRejection = /cannot|can't|unable|sorry|policy|violat/i.test(
                    conversationText
                );
                if (isContentRejection) {
                    logger.warn(
                        "适配器",
                        `早期检测到内容拒绝: ${conversationText.substring(0, 200)}...`,
                        meta
                    );
                    return {
        ...
        try {
            ...
            logger.debug(
                "适配器",
                `等待图片超时, conversationText长度: ${conversationText.length}, downloadUrl: ${downloadUrl}`,
                meta
            );
            ...
            if (conversationText && !downloadUrl) {
                ...
                logger.warn(
                    "适配器",
                    `模型返回文本而非图片: ${conversationText.substring(0, 200)}...`,
                    meta
                );
                return {
                    error: `模型返回文本而非图片: ${conversationText.substring(0, 200)}`,
                    retryable: false
                };
            }
            ...
            if (!conversationText && conversationBody) {
                ...
                if (partsMatch && partsMatch[1]) {
                    ...
                    return {
                        error: `模型返回文本而非图片: ${partsMatch[1].substring(0, 200)}`,
                        retryable: false
                    };
                }
            }
...
export const manifest = {
    ...
    description:
        "使用 ChatGPT 官网生成图片，支持参考图片上传。需要已登录的 ChatGPT 账户，请使用会员账号 (包含 K12 教师认证)，非会员账号会有速率限制。",
