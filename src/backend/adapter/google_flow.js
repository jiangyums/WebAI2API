...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
    ...
    try {
        ...
        if ((await modelCombobox.count()) > 0) {
            ...
            await sleep(300, 500);
            await safeClick(page, page.getByRole("option", { name: codeName, exact: true }), {
                bias: "button"
            });
        ...
        const sizeCombobox = page.getByRole("combobox").filter({ hasText: /16:9|9:16/ });
...
export const manifest = {
    ...
    description:
        "使用 Google Labs Flow 工具生成图片，支持多张参考图片上传和横竖版选择。需要已登录的 Google 账户。",
